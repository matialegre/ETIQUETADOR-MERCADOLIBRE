...
def list_orders_with_filters():
    """
    Lista órdenes con filtros por ARTICULO, COLOR, TALLE y COMENTARIO.
    Query params soportados: articulo, color, talle, comentario, limit (default 200)
    """
    try:
        articulo = (request.args.get('articulo') or '').strip()
        color = (request.args.get('color') or '').strip()
        talle = (request.args.get('talle') or '').strip()
        comentario = (request.args.get('comentario') or '').strip()
        try:
        ...
        params = []
        if articulo:
            where_clauses.append("ARTICULO = ?")
            params.append(articulo)
        if color:
            where_clauses.append("COLOR = ?")
            params.append(color)
        if talle:
            where_clauses.append("TALLE = ?")
            params.append(talle)
        if comentario:
            where_clauses.append("COMENTARIO LIKE ?")
            params.append(f"%{comentario}%")
        ...
        with pyodbc.connect(CONN_STR) as conn:
            cursor = conn.cursor()
            sql = f"""
                SELECT TOP (?) order_id, sku, qty, ARTICULO, COLOR, TALLE,
                               COMENTARIO, producto_titulo, estado, subestado,
                               shipping_estado, fecha_actualizacion
                FROM orders_meli
                {where_sql}
            ...
            for r in rows:
                orders.append({
                    ...
                    'estado': r[8],
                    'subestado': r[9],
                    'shipping_estado': r[10],
                    'fecha_actualizacion': r[11].isoformat() if r[11] else None
                })

        log_json('api_orders_list', {
            'filters': { 'articulo': articulo, 'color': color, 'talle': talle, 'comentario': bool(comentario) },
            'count': len(orders)
        })
...
@app.route('/api/orders/<order_id>/comentario', methods=['POST'])
def set_order_comentario(order_id: str):
    """
    Actualiza el campo COMENTARIO de una orden.
    Body JSON: { "comentario": "texto" }
    """
    try:
        data = request.get_json(force=True)
        comentario = (data or {}).get('comentario', '')
        ...
        log_json('order_comentario_updated', {
            'order_id': order_id,
            'len': len(comentario or '')
        })

        # Notificar por WebSocket opcionalmente
        broadcast({
            'type': 'order.comentario.updated',
            'order_id': order_id
        })

        return jsonify({'status': 'success', 'order_id': order_id, 'comentario': comentario})

    except Exception as e:
        log_json('api_error', {
            'endpoint': f'/api/orders/{order_id}/comentario',
            'error': str(e)
        })
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
if __name__ == "__main__":
    ...
    print("  GET  /api/orders?articulo=&color=&talle=&comentario=")
