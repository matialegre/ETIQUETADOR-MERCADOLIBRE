...
from sqlalchemy import Column, Integer, String, DateTime, Boolean, Text, Decimal, BigInteger
...
class OrderItem(Base):
    ...
    # Movimiento WOO→WOO (Dragonfish)
    # Mapeamos a los nombres reales de la tabla:
    #  - movimiento_realizado (bit)
    #  - observacion_movimiento (nvarchar(500))
    #  - numero_movimiento (nvarchar(100))
    #  - fecha_actualizacion (datetime2)
    movimiento_realizado = Column(Boolean, default=False)
    movimiento_fecha = Column('fecha_actualizacion', DateTime)
    movimiento_nota = Column('observacion_movimiento', String(500))
    movimiento_numero = Column('numero_movimiento', String(100))
