...
# ───────── CONFIG ───────────────────────────────────────────────
CLIENT_ID      = "8063857234740063"
CLIENT_SECRET  = "sV4A7MufbYvL58rzg9W56Va8u36OlsIb"
...
def _app_dir() -> Path:
    ...
    - En PyInstaller: carpeta donde está el .exe (no _MEIPASS temporal).
    """
    if getattr(sys, 'frozen', False):
        return Path(sys.executable).parent
    return Path(__file__).parent

# Archivos junto al exe/py
APP_DIR = _app_dir()
TOKEN_PATH      = APP_DIR / "token_caba.json"
...
def token(force: bool = False) -> Tuple[str, str]:
    ...
    # Si created_at falta o es 0, o expires_in <= 0 → refrescar si es posible
    if int(t.get("created_at", 0)) <= 0 or int(t.get("expires_in", 0)) <= 0:
        if "refresh_token" in t:
            t = refresh_token(t)  # setea created_at y persiste
        else:
            # Fallback: fijar created_at antiguo para forzar refresh posterior
...
def mover_stock(oid: int,
    ...
    for sku, qty in req.items():
        ...
        ok_env, mov_id = enviar_movimiento_stock(oid, codb, qty, info)
        print(f"         ← POST ok={ok_env} mov={mov_id or '-'}"); sys.stdout.flush()
        ...
        if not ok_env:
            ...
            total_desc += qty
            if mov_id:
                mov_ids.append(str(mov_id))

    if ok_global and total_desc:
        append_stock_note(oid, tok, total_desc, mov_ids if mov_ids else None)
...
def enviar_movimiento_stock(pedido: int, codb: str, cant: int,
    ...
    if r.status_code == 201:
        mov_id = None
        # intentar extraer el número de movimiento del body
        try:
            j = r.json()
            # nombres probables
        ...
        if not mov_id:
            ...
            m = re.search(r"\b(\d{5,})\b", r.text or "")
            mov_id = m.group(1) if m else None
        if VERBOSE:
            preview = (r.text or "")[:300]
            print(f"[DF] OK 201: {preview} – mov={mov_id or '-'}")
        return True, mov_id

    # Duplicado
    if r.status_code == 409:
        print("[DF] 409 Ya existía → lo doy por válido")
        # si viene un body, intento extraer ID también
        mov_id = None
        try:
            j = r.json()
            for k in ("Numero", "NroMovimiento", "Id", "IdMovimiento", "NumeroMovimiento"):
...
STOCK_TAG_RE = re.compile(r"\[STOCK[^\]]*\]", re.I)
def append_stock_note(oid: int, tok: str, qty: int, mov_ids: list[str] | None = None) -> None:
    nota = _nota_primera(
    ...
    if mov_ids:
        mov_tag = (f"MOV {mov_ids[0]}" if len(mov_ids) == 1
                   else f"MOVS {','.join(mov_ids)}")
        nuevo = f"{txt.strip()} [STOCK -{qty} {mov_tag}]"
    else:
        nuevo = f"{txt.strip()} [STOCK -{qty}]"

    # Agregar 'MUNDOCAB' si no está presente aún
    if 'mundocab' not in nuevo.lower():
        nuevo = f"{nuevo} MUNDOCAB"
...
def procesar_stock_si_corresponde(oid:int, req:dict[str,int], tok:str, nota_prev:str)->None:
    ...
    for sku,qty in req.items():
        ...
        ok_env, _mov = enviar_movimiento_stock(oid, codb, qty, info)
        if ok_env:
            total+=qty
    if total:
...
def ciclo() -> None:
    ...
    for pid, lote in packs.items():
        ...
        try:
            ...
            # ---------- 2.3.b Filtrar por estado del envío ------------------
            # Sólo procesar cuando la etiqueta está lista/impresa
            estados_permitidos = {"Etiqueta lista para imprimir", "Etiqueta impresa"}
            if estado not in estados_permitidos:
                if VERBOSE:
                    print(f"   ↷ Salteado por estado: {estado}")
                continue
            ...
            # Marcar nueva venta como DESCONTADO (incluye MUNDOCAB) y mover stock
            if not upsert_replace_api(real_id, tok, "[API: DESCONTADO] MUNDOCAB"):
