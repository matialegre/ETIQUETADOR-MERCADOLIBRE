"""Fase 3 – GUI CABA simplificada mejorada con tabla usando ttkbootstrap.
...
VERSIÓN CABA: Configurada para servidor remoto y filtros CAB/CABA/MUNDOCAB.
...
# CONFIGURACIÓN CABA: Aplicar configuraciones específicas ANTES de importar otros módulos
import os
os.environ['CABA_VERSION'] = 'true'  # CRÍTICO: Configurar variable de entorno para picker_service
import config_override_caba  # Esto aplica automáticamente la configuración CABA
...
# CONFIGURACIÓN CABA: Filtros específicos para CABA
...
class AppV3(tb.Window):
    ...
    def __init__(self) -> None:
        super().__init__(title="Cliente Matías – CABA – Tabla Optimizada", themename="darkly")
    ...
    def _build_widgets(self) -> None:
        ...
        # Nuevo botón para reimprimir etiqueta (solo para pedidos impresos)
        self.btn_reprint = tb.Button(frm_top, text="REIMPRIMIR ETIQUETA", bootstyle=WARNING, command=self.on_reprint, state=DISABLED)
        self.btn_reprint.grid(row=0, column=13, padx=4)
        ...
        try:
            ...
            print(f"🔍 DEBUG: Buscando imagen de fondo...")
            print(f"📁 Directorio del ejecutable/script: {script_dir}")
            print(f"📁 Directorio temporal: {temp_dir}")
            print(f"🔍 Rutas a verificar:")
            ...
            if bg_path:
                print(f"✅ IMAGEN ENCONTRADA: {bg_path}")
                try:
                    # Intentar cargar imagen con PIL para mejor compatibilidad
                    print(f"📎 Intentando cargar con PIL...")
                    from PIL import Image, ImageTk
                    print(f"✅ PIL importado correctamente")
                    
                    # Cargar y redimensionar imagen
                    print(f"📎 Abriendo imagen: {bg_path}")
                    ...
                    print(f"📎 Aplicando transparencia...")
                    ...
                    print(f"📎 Creando PhotoImage...")
                    self.bg_image = ImageTk.PhotoImage(pil_image)
                    print(f"📎 Creando Label de fondo...")
                    ...
                    print(f"📎 Label creado y posicionado")
                    ...
                    self.bg_label.lower()
                    print(f"✅ Imagen de fondo cargada con PIL: {bg_path} ({frame_width}x{frame_height})")
                    print(f"🎉 ¡IMAGEN DE FONDO APLICADA EXITOSAMENTE!")
                    
                    # Forzar actualización de la ventana
                    self.update_idletasks()
                    print(f"🔄 Ventana actualizada")
    ...
    def populate_tree(self, orders):
        ...
        # Multipack = packs con múltiples órdenes O packs consolidados
        multipack_ids = {pack_id for pack_id, count in pack_counts.items() if count > 1} | consolidated_pack_ids
    ...
    def on_tree_select(self, event):
        ...
        # Por defecto, deshabilitar reimpresión
        self.btn_reprint.configure(state=DISABLED)
        # Habilitar reimpresión solo si el seleccionado corresponde a un pedido IMPRESO
        if sel:
            try:
                iid = sel[0]
                ...
                for o in self.state.visibles:
                    if str(o.pack_id or o.id) == ord_id:
                        sub = (o.shipping_substatus or '').lower()
                        if sub == 'printed' and getattr(o, 'shipping_id', None):
                            self.btn_reprint.configure(state=NORMAL)
                        break
            except Exception:
                # Si hay error al resolver, mantener deshabilitado
                self.btn_reprint.configure(state=DISABLED)
    ...
    def on_reprint(self):
        """Reimprime la etiqueta de la venta seleccionada si está en estado PRINTED."""
        sel = self.tree.selection()
        if not sel:
            return
        iid = sel[0]
        ...
        for o in self.state.visibles:
            if str(o.pack_id or o.id) == ord_id:
                ...
                break
        if not target_order:
            dialogs.Messagebox.show_error("No se encontró la orden seleccionada")
            return
        sub = (target_order.shipping_substatus or '').lower()
        if sub != 'printed':
            dialogs.Messagebox.show_info("Solo se puede reimprimir etiquetas de ventas con estado IMPRESO (PRINTED)")
            return
        shipping_id = getattr(target_order, 'shipping_id', None)
        if not shipping_id:
            dialogs.Messagebox.show_error("La orden seleccionada no tiene shipping_id válido")
            return
        try:
            ok, msg = self.picker.print_shipping_label_with_retries(shipping_id)
            if ok:
                dialogs.Messagebox.show_info("Etiqueta reimpresa correctamente")
            else:
                dialogs.Messagebox.show_error(f"No se pudo reimprimir: {msg}")
        except Exception as e:
            dialogs.Messagebox.show_error(f"Error reimprimiendo etiqueta: {e}")
    ...
    def _send_whatsapp_notification(self, data: Dict[str, Any], motivo: str) -> None:
        """Envía mensaje WhatsApp vía CallMeBot con datos del ítem fallado (solo CABA).

        Incluye link clickeable para contactar al 11 5863-3575 con mensaje prellenado.
        """
        try:
            art = str(data.get('art') or '')
            color = str(data.get('color') or '')
            talle = str(data.get('talle') or '')
            order_num = str(data.get('pack_id') or data.get('order_id') or '')

            # Link a WhatsApp de contacto con texto prellenado
            contact_text = f"Consulta por {art} - Color: {color} - Talle: {talle} - Motivo FALLADO: {motivo}"
            contact_link = f"https://wa.me/541158633575?text={quote_plus(contact_text)}"

            # Mensaje principal a enviar por CallMeBot
            body = (
                "FALLADO CABA QR\n"
                f"Pedido: {order_num}\n"
                f"Artículo: {art}\n"
            ...
            params = {
                "phone": "5492914656671",  # Destino CallMeBot
                "text": body,
                "apikey": "2269471",
            }
            resp = requests.get(url, params=params, timeout=10)
            if resp.status_code != 200:
                log.warning("WhatsApp API status %s: %s", resp.status_code, resp.text[:200])
            else:
                log.info("WhatsApp notificación enviada para pedido %s", order_num)
        except Exception as e:
            log.warning("Error enviando notificación WhatsApp: %s", e)
                ...
                for o in self.state.visibles:
                    if (o.pack_id or o.id) == data["pack_id"] or o.id == data["order_id"]:
                        try:
                            ...
                            pass
                # Enviar notificación por WhatsApp (CallMeBot) con los datos y motivo
                self._send_whatsapp_notification(data, motivo_var.get())
    ...
    def on_print_list(self):
        ...
        try:
            # Determinar título según cuenta (usamos BANNER_TEXT configurado por el launcher)
            banner = (os.environ.get('BANNER_TEXT') or '').upper()
            if 'CORREO' in banner or 'QR' in banner:
                title = 'PAQUETERÍA CORREO (CÓDIGO QR)'
            else:
                title = 'PAQUETERÍA ANDREANI (CÓDIGO DE BARRA)'

            print_service.print_list_pdf(rows, config.PRINTER_NAME, title=title)
    ...
    def _show_new_sale_notification(self, new_orders):
        ...
        try:
            ...
            # Filtrar órdenes que corresponden a este depósito (CABA)
            filtered_orders = []
            ...
            # Si no hay órdenes relevantes para CABA, no mostrar notificación
            if not filtered_orders:
                return
            ...
            notification_win.title("🎉 ¡Nueva Venta CABA!")
            ...
            title_label = tb.Label(
                main_frame, 
                text="🎉 ¡NUEVA VENTA CABA DETECTADA!", 
                font=("Arial", 18, "bold"),
                bootstyle="success"
            )
            ...
            if len(filtered_orders) > 2:
                more_orders_label = tb.Label(
                    main_frame,
                    text=f"... y {len(filtered_orders) - 2} pedido(s) más para CABA",
...
def launch_gui_v3_caba():
    ...
    print("🏢 Iniciando Cliente Matías - Versión CABA")
    print(f"📡 API Dragonfish: {config_override_caba.os.environ.get('DRAGONFISH_BASE_URL', 'NO CONFIGURADA')}")
    print(f"🗄️ SQL Server: {config_override_caba.os.environ.get('SQL_SERVER', 'NO CONFIGURADO')}")
    print(f"🏷️ Filtros: {KEYWORDS_NOTE}")
