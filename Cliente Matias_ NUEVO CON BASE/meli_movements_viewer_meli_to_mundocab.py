#!/usr/bin/env python3
# ────────────────────────────────────────────────────────────────
#  meli_movements_viewer_meli_to_mundocab.py
#  Visor especializado para movimientos BASE=MELI → DESTINO=MUNDOCAB
#  Filtros predeterminados: BaseDeDatosAltaFW=MELI, OrigenDestino=MUNDOCAB, Fecha seleccionada
# ────────────────────────────────────────────────────────────────
...
class MeliToMundocabViewer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Visor de Movimientos MELI → MUNDOCAB")
    ...
    def setup_main_window(self):
        ...
        title_label = tk.Label(
            title_frame,
            text="🛒 VISOR MOVIMIENTOS MELI → MUNDOCAB",
            font=("Arial", 22, "bold"),
            bg='#f0f0f0',
            fg='#e74c3c'
        )
        ...
        subtitle_label = tk.Label(
            title_frame,
            text="Filtros automáticos: BASE=MELI → DESTINO=MUNDOCAB | FECHA SELECCIONADA",
            font=("Arial", 14),
            bg='#f0f0f0',
            fg='#7f8c8d'
        )
        ...
        info_label = tk.Label(
            info_frame,
            text="🔒 FILTROS FIJOS: BaseDeDatosAltaFW=MELI | OrigenDestino=MUNDOCAB",
            font=("Arial", 12, "bold"),
            bg='#ecf0f1',
            fg='#2c3e50',
            pady=10
        ...
        self.status_label = tk.Label(
            self.root,
            text=f"Listo para buscar movimientos MELI → MUNDOCAB del {self.current_date.strftime('%d/%m/%Y')}",
            font=("Arial", 11),
            bg='#f0f0f0',
            fg='#7f8c8d'
        )
    ...
    def update_date_display(self):
        ...
        self.status_label.config(text=f"Listo para buscar movimientos MELI → MUNDOCAB del {date_str}")
        
    def search_current_date_movements(self):
        date_str = self.current_date.strftime("%Y-%m-%d")
        self.status_label.config(text=f"🔍 Buscando movimientos MELI → MUNDOCAB del {self.current_date.strftime('%d/%m/%Y')}...")
        self.root.update()
    ...
    def show_results(self, movements, date_from, date_to):
        if not movements:
            messagebox.showinfo("Sin resultados", "No se encontraron movimientos MELI → MUNDOCAB para la fecha especificada.")
            self.status_label.config(text="Sin resultados encontrados")
            return
        
        results_window = tk.Toplevel(self.root)
        results_window.title(f"Movimientos MELI → MUNDOCAB - {len(movements)} resultados")
        ...
        title_label = tk.Label(
            title_frame,
            text=f"🛒 Movimientos MELI → MUNDOCAB ({len(movements)} resultados)",
            font=("Arial", 18, "bold"),
            bg='#ffffff',
            fg='#e74c3c'
        )
        ...
        filters_text = f"📅 Fecha: {date_from} | 🏢 MELI → MUNDOCAB"
        ...
        self.status_label.config(text=f"✅ {len(movements)} movimientos MELI → MUNDOCAB encontrados")
    
    def run(self):
        self.root.mainloop()

if __name__ == "__main__":
    app = MeliToMundocabViewer()
    app.run()
