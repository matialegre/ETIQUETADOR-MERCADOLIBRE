...
class MeliMovementsViewer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Visor de Movimientos MUNDOCAB → MELI (CABA)")
        self.root.geometry("1400x900")
        self.root.state('zoomed')  # Maximizar ventana en Windows
        self.root.configure(bg='#f0f0f0')
        
        # Configuración de la API
        self.api_base = "http://190.211.201.217:8009/api.Dragonfish"
        self.headers = {
            ...
            "BaseDeDatos": "MELI"  # Cambiar a MELI para buscar movimientos que entran a MELI desde CABA
        }
    ...
    def setup_main_window(self):
        ...
        title_label = tk.Label(
            title_frame,
            text="🏢 VISOR MOVIMIENTOS MUNDOCAB → MELI (CABA)",
            font=("Arial", 22, "bold"),
            bg='#f0f0f0',
            fg='#e74c3c'
        )
        ...
        info_label = tk.Label(
            info_frame,
            text="🔒 FILTROS FIJOS: BaseDeDatos=MELI, OrigenDestino=MUNDOCAB, Motivo=API, Vendedor=API, Tipo=ENTRADA",
            font=("Arial", 12, "bold"),
            bg='#ecf0f1',
            fg='#2c3e50',
            pady=10
    ...
    def update_date_display(self):
        ...
        self.status_label.config(text=f"Listo para buscar movimientos MUNDOCAB → MELI del {date_str}")
        
    def search_current_date_movements(self):
        """Busca movimientos para la fecha actual seleccionada"""
        date_str = self.current_date.strftime("%Y-%m-%d")
        
        self.status_label.config(text=f"🔍 Buscando movimientos MUNDOCAB → MELI del {self.current_date.strftime('%d/%m/%Y')}...")
        self.root.update()
        
        # Ejecutar búsqueda en hilo separado
        threading.Thread(
            target=self.fetch_movements,
            args=(date_str, date_str, "API", "MUNDOCAB"),
            daemon=True
        ).start()
    ...
    def search_today_movements(self):
        ...
        self.status_label.config(text="🔍 Buscando movimientos MUNDOCAB → MELI...")
        self.root.update()
        
        # Ejecutar búsqueda en hilo separado
        threading.Thread(
            target=self.fetch_movements,
            args=(today, today, "API", "MUNDOCAB"),
            daemon=True
        ).start()
    ...
    def fetch_movements(self, date_from, date_to, vendor, origen_destino):
        ...
        try:
            ...
            while current_date <= end_date:
                ...
                # Parámetros para buscar movimientos MUNDOCAB → MELI (CABA)
                # Según el código: OrigenDestino="MUNDOCAB", Tipo=1, BaseDeDatosAltaFW="MELI"
                params = {
                    "Fecha": date_str,
                    "vendedor": vendor,
                    "OrigenDestino": origen_destino,  # "MUNDOCAB"
                    "limit": 1000
                ...
                try:
                    ...
                    for i, movement in enumerate(movements):
                        ...
                        # Filtros correctos para MUNDOCAB → MELI (CABA):
                        # - Motivo: "API"
                        # - Tipo: 1 (Entrada)
                        # - Vendedor: "API" 
                        # - OrigenDestino: "MUNDOCAB"
                        # - BaseDeDatosAltaFW: "MELI" (indica que entra a MELI)
                        motivo_ok = motivo == 'API'
                        tipo_ok = tipo == 1  # 1 = Entrada (MUNDOCAB → MELI)
                        vendedor_ok = vendedor_mov == 'API'
                        destino_ok = origen_destino == 'MUNDOCAB'
                        base_ok = base_datos_movimiento == 'MELI'  # Entra a MELI
    ...
    def show_results(self, movements, date_from, date_to):
        ...
        # Actualizar status
        self.status_label.config(text=f"✅ {len(movements)} movimientos MUNDOCAB → MELI encontrados")
